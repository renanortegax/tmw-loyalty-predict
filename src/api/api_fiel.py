...
#  mlflolw sever
#  flask --app src/api/api_fiel.py run --port 5001 --debug
import mlflow
...
def predict():
    try:
        ...
        return {"idcliente":data['idcliente'], "result":float(predict)}
        
    except Exception as e:
        return {"error":f"Verifique os formato dos dados enviados.\n Erro: {repr(e)}"}, 400
...
def predict_many():
    try:
        ...
        return {"error":f"Verifique os formato dos dados enviados. Erro: {repr(e)}"}, 400